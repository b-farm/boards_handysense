...
/* Sw on HandySense */
int sw_onboard[4] = {36, 39, 34, 35};
/* const relay pin */
const int const_relay_pin[4]  = {32, 33, 25, 26};
...
int check_SetThreshold = 200;
int ActivethresholdRalay[2] = {0, 1};
int dayofweekformblock[8] = {0, 1, 2, 3, 4, 5, 6, 7};
int old_time = 0;

/* ------------- Read Analog from MCP3424 (Alone) -----------*/
float ReadAnalog_MPC3424 (int ch) {
  MCP_soil.Configuration(ch, 16, 0, 1);
  float buffer = 0, Microvolt;  
  MCP_soil.NewConversion();
  Microvolt = MCP_soil.Measure();
  Serial.println("Microvolt : " + String(Microvolt) + " Microvolt");
  return Microvolt;
}

/* ------------- Read Analog from MCP3424 MAP function -----------*/
float ReadAnalog_from_MPC3424 (int ch, int valueMin, int valueMax, int OutMin, int OutMax) {
  MCP_soil.Configuration(ch, 16, 0, 1);
  float buffer = 0, Microvolt;  
  MCP_soil.NewConversion();
  Microvolt = MCP_soil.Measure();
  Serial.println("Microvolt : " + String(Microvolt) + " Microvolt");
  buffer = map(Microvolt, valueMin, valueMax, OutMin, OutMax);
  return buffer;
}

/* ------------- Read 4-20mA from MCP3424 (Alone) -----------*/
float Read4_20mA_MPC3424 (int ch) {
  MCP_soil.Configuration(ch, 16, 0, 1);
  float buffer = 0, Microvolt;  
  MCP_soil.NewConversion();
  Microvolt = MCP_soil.Measure();
...
/* ------------- Read Analog from MCP3424 MAP function -----------*/
float Read4_20mA_MPC3424_map (int ch, int valueMin, int valueMax, int OutMin, int OutMax) {
  MCP_soil.Configuration(ch, 16, 0, 1);
  float buffer = 0, Microvolt;  
  MCP_soil.NewConversion();
  Microvolt = MCP_soil.Measure();
...
void ControlRelay_Bymanual(String topic, String message, unsigned int length)
{
  ...
  DEBUG_PRINTLN(manual_message);
  DEBUG_PRINT("manual_relay   : ");
  DEBUG_PRINTLN(manual_relay);
...
void Get_sht31(int Raw_Temp, int Raw_Hum)
{
  ...
  buffer_hum = Raw_Hum;
  if (buffer_temp < -40 || buffer_temp > 125 || isnan(buffer_temp))
  { // range -40 to 125 C
  ...
  else
  {
    ...
    temp_from_Sensor = Raw_Temp; //temp_cal / num_temp;
    temp_error = 0;
    temp_error_count = 0;
  }
  float hum_cal = 0;
  int num_hum = 0;
  if (buffer_hum < 0 || buffer_hum > 100 || isnan(buffer_hum))
  { // range 0 to 100 %RH
  ...
  else
  {
    ...
    humidity_from_Sensor = Raw_Hum; //hum_cal / num_hum;
    hum_error = 0;
    hum_error_count = 0;
  }
...
void Get_light_sensor(int Raw_Light)
{
  ...
  if (buffer_lux < 0 || buffer_lux > 188000 || isnan(buffer_lux))
...
{
  float buffer_soil = 0;
  float read_data = analogRead(35);
  read_data = (read_data * 5.0) / (4095.00);
  buffer_soil = ((0.8) * read_data) - 0.4;
  //buffer_soil = Raw_soil;
  //buffer_soil = read_data;
  DEBUG_PRINT("read_data: "); DEBUG_PRINTLN(read_data);
  DEBUG_PRINT("buffer_soil: "); DEBUG_PRINTLN(buffer_soil);

  if (buffer_soil < 0 || buffer_soil > 100 || isnan(buffer_soil))
  { // range 0 to 100 %
...
void setup_HandySense() {
  ...
  pinMode(sw_onboard[0], INPUT);
  pinMode(sw_onboard[1], INPUT);
  pinMode(sw_onboard[2], INPUT);
  pinMode(sw_onboard[3], INPUT);

  pinMode(const_relay_pin[0], OUTPUT);
  pinMode(const_relay_pin[1], OUTPUT);
  pinMode(const_relay_pin[2], OUTPUT);
  pinMode(const_relay_pin[3], OUTPUT);
...
void loop_HandySense(float Soil_RawData, float brightness_RawData, float Temp_RawData, int Hum_RawData) {
  if (state_timer_of_connect_wifi == 0) {
    timer_of_connect_wifi = millis();
    state_timer_of_connect_wifi = 1;
  }
  ...
  if (!isnan(Soil_RawData) && !isnan(brightness_RawData) && !isnan(Temp_RawData) && !isnan(Hum_RawData)) {
    ...
    if (currenttime_HS - previousTime_Temp_soil >= eventInterval) {
      ...
      if (millis() > 60000)
      { // dylay 0 s to start check soil diff
      ...
      DEBUG_PRINTLN("");
      DEBUG_PRINT("Temp : ");
      DEBUG_PRINT(temp_from_Sensor);
      DEBUG_PRINT(" C, ");
      DEBUG_PRINT("Hum  : ");
      DEBUG_PRINT(humidity_from_Sensor);
      DEBUG_PRINT(" %RH, ");
      DEBUG_PRINT("Brightness : ");
      DEBUG_PRINT(lux_from_Sensor);
      DEBUG_PRINT(" Klux, ");
      DEBUG_PRINT("Soil  : ");
      DEBUG_PRINT(soil_from_Sensor);
      DEBUG_PRINTLN(" %");
