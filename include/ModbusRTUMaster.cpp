...
void ModbusRTUMaster::begin(uint32_t baud, uint8_t config) {
  if (_hardwareSerial) {
...
void ModbusRTUMaster::_writeRequest(uint8_t len) {
  ...
  if (Serial) {
	Serial.println("Print from _writeRequest: ");
	for (int i=0; i<len; i++) {
		Serial.println(_buf[i]);
	}
	Serial.println("Ended from _writeRequest");
...
void ModbusRTUMaster::_calculateTimeouts(uint32_t baud, uint8_t config) {
